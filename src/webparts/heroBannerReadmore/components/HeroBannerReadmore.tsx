...
import styles from './HeroBannerReadmore.module.scss';
import { IHeroBannerReadMoreProps } from './IHeroBannerReadmoreProps';
...
export default class HeroBannerRm extends React.Component<IHeroBannerReadMoreProps, IHeroBannerRmState, {}> {
  constructor(props: IHeroBannerReadMoreProps, state: IHeroBannerRmState) {
    super(props);
    this.state = {
      Items: [],
      ItemID: null,
  ...
  public pagereload() {
    ...
    const nextTitle = 'Hero-Banner-ReadMore';
  ...
  public async GetBannerDetails(ItemID) {
    var reactHandler = this;
    await sp.web.lists.getByTitle("Hero Banner").items.select("Title", "EnableComments", "EnableLikes", "Description", "Created", "Image", "ID").filter(`IsActive eq '1' and ID eq '${ItemID}'`).getAll().then((items) => { // //orderby is false -> decending          
      title = items[0].Title;
      ID = items[0].ID
      this.setState({
  ...
  public checkUserAlreadyLiked() {
    sp.web.lists.getByTitle("LikesCountMaster").items.filter(`ContentPage eq 'Hero-Banner' and ContentID eq ${ID} and EmployeeName/Id eq ${User}`).top(5000).get().then((items) => { // //orderby is false -> decending          
      if (items.length != 0) {
        $(".like-selected").show();
        $(".like-default").hide();
        this.setState({
  ...
  public checkUserAlreadyCommented() {
    sp.web.lists.getByTitle("CommentsCountMaster").items.filter(`ContentPage eq 'Hero-Banner' and ContentID eq ${ID} and EmployeeName/Id eq ${User}`).top(5000).get().then((items) => { // //orderby is false -> decending          
      if (items.length != 0) {
        this.setState({
  ...
  public commentsCount() {
    sp.web.lists.getByTitle("CommentsCountMaster").items.filter(`ContentPage eq 'Hero-Banner' and ContentID eq ${ID}`).top(5000).get().then((items) => { // //orderby is false -> decending          
      if (items.length != 0) {
  ...
  public async liked(mode) {
    ...
    if (mode == "like") {
      ...
      $(".like-selected").hide();
      $(".like-default").show();
      sp.web.lists.getByTitle("LikesCountMaster").items.filter(`ContentPage eq 'Hero-Banner' and ContentID eq ${ID} and EmployeeName/Id eq ${User}`).get().then((data) => {
